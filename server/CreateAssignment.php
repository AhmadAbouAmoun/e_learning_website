...
if (!isset($input["id"]) || !isset($input["title"]) || !isset($input["description"]) || !isset($input["course_id"])) {
...
$query = $connection->prepare("INSERT INTO assignments (id, title, description, course_id) VALUES (?, ?, ?, ?)");
if (!$query) {
    echo json_encode(["status" => "failed", "message" => "Query preparation failed: " . $connection->error]);
    return;
}

$query->bind_param("issi", $id, $title, $description, $course_id);
